...
$conn = mysqli_connect("localhost", "root", "", "sportcenter");
...
function daftar($data)
{
  ...
  mysqli_query($conn, "INSERT INTO user_212279 (212279_email,212279_password,212279_no_handphone,212279_jenis_kelamin,212279_username,212279_alamat,212279_foto) VALUES ('$username','$password','$no_handphone','$gender','$nama','$alamat','$upload')");
  return mysqli_affected_rows($conn);
}
...
function pesan($data)
{
    ...
    // Periksa apakah ada pemesanan lain yang bertabrakan dengan waktu yang diinginkan
    $query = "SELECT * FROM sewa_212279 WHERE 212279_id_lapangan = ? AND (
                (? BETWEEN 212279_jam_mulai AND 212279_jam_habis) OR 
                (? BETWEEN 212279_jam_mulai AND 212279_jam_habis) OR
                (212279_jam_mulai BETWEEN ? AND ?) OR
                (212279_jam_habis BETWEEN ? AND ?)
              )";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("sssssss", $idlpg, $mulai, $habis, $mulai, $habis, $mulai, $habis);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        // Jika tidak ada bentrokan, lakukan pemesanan
        $query = "INSERT INTO sewa_212279 (212279_id_user, 212279_id_lapangan, 212279_lama_sewa, 212279_jam_mulai, 212279_jam_habis, 212279_harga, 212279_total) 
                  VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("iisssii", $userid, $idlpg, $lama, $mulai, $habis, $harga, $total);
        $stmt->execute();
